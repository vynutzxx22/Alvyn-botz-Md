...
Tol = await getBuffer(`https://hardianto.xyz/api/goodbye3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://telegra.ph/file/8bbe8a7de5c351dfcb077.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)
                if (anu.action == 'add') {
                    GojoMdNx.sendMessage(anu.id, { image: Kon, contextInfo: { mentionedJid: [num] }, caption: `Selamat Datang 🥳 @${num.split("@")[0]}, digrup ${metadata.subject}\n\nBaca deskripsi grup dibawah ini\n${metadata.desc}\n\nSilahkan intro memperkenalkan diri @${num.split("@")[0]},\n♨️ Nama : \n🔞 Umur : \n👥 Gender : \n🏙️ Askot : \n\nPatuhi peraturan grup ya demi kenyamanan bersama\nSemoga betah digrup ini 🤗`} )
                } else if (anu.action == 'remove') {
                    GojoMdNx.sendMessage(anu.id, { image: Tol, contextInfo: { mentionedJid: [num] }, caption: `Selamat Tinggal Beban Grup 😭🔫😤🦶 @${num.split("@")[0]}, Keluar dari grup ${metadata.subject}\n\nJangan semangat tetap putus asa dan jadilah beban keluarga 😭🗿` })
                }
            }
        } catch (err) {
            console.log(err)
	...
	for (let i of kon) {
	    list.push({
	    	...
	    	vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${ownername}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:💬 Click To Chat\nitem2.URL;type=INTERNET:${sc}\nitem2.X-ABLabel:🌐 My Website Profile\nitem3.URL:${ig}\nitem3.X-ABLabel:📢 My Instagram\nitem4.ADR:;;${region};;;;\nitem4.X-ABLabel:🏠 My House\nEND:VCARD`
	    })
	}
