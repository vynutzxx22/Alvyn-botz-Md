...
async function startGojoMdNx() {
    ...
    GojoMdNx.sendMessage(callerId, { text: `Sistem Otomatis Blokir!\nJangan Menelpon Bot!\nChat Owner Untuk Dibuka Blokirnya\n\nKontak Yang Dikirim Ini Bukan Bot, Tapi Yang Punya Bot Ini\nChat Untuk Buat Stiker Atau Chat Yang Gak Jelas Akan Langsung Diblokir`}, { quoted : pa7rick })
    await sleep(8000)
...
Tol = await getBuffer(`https://hardianto.xyz/api/goodbye3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://telegra.ph/file/8bbe8a7de5c351dfcb077.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)
                ...
                GojoMdNx.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `┌─❖
│「 𝗛𝗶 👋 」
└┬❖ 「 @${xeonName.split("@")[0]}  」
